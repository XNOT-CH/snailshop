...
function createRequest() {
    return new NextRequest("http://localhost/api/auth/send-verification-email", {
        method: "POST",
    });
}

describe("API: /api/auth/send-verification-email", () => {
    beforeEach(() => {
        vi.clearAllMocks();
    ...
    it("rejects requests that fail CSRF-backed authentication before sending email", async () => {
        ...
        const { POST } = await import("@/app/api/auth/send-verification-email/route");

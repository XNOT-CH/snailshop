...
// The money test. Signups and top-up baht live at two different grains, so
// counting them in one join multiplies them by each other: a promoter with
// 2 signups and 3 top-ups would be credited 6 times over.
...
describe("listInviteCodesWithStats", () => {
    ...
    it("counts each metric with its own query rather than one join", async () => {
        ...
        // Two selects: signups, baht. A single joined query is the fan-out bug
        // this whole shape exists to avoid.
        expect(selectCalls).toHaveLength(2);
    });

    it("shows a code nobody has used yet as zeros instead of dropping it", async () => {
        selectResults.push([{ inviteCodeId: "invite-1", signups: 5 }], []);
    ...
    it("filters signups by the date range but never the top-up total", async () => {
        ...
        const [signupWhere, topupWhere] = whereCalls.map((condition) =>

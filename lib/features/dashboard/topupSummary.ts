...
export function parseSelectedDays(raw: string | null) {
    if (!raw) return null;
...
export function matchesDateRange(value: string | Date, start: Date, end: Date) {
    const current = new Date(value);
    return current >= start && current <= end;
}

export function matchesDayFilter(value: string | Date, selectedDays: number[] | null) {
...
export async function getDashboardTopupSummary(searchParams: URLSearchParams) {
    ...
    const rawPageSize = Number.parseInt(searchParams.get("pageSize") || "10", 10) || 10;
    const page = Math.max(1, rawPage);
    const pageSize = Math.min(50, Math.max(1, rawPageSize));
    const search = searchParams.get("search")?.trim() || "";
    const status = searchParams.get("status") || "ALL";
    const sortKey = searchParams.get("sortKey") || "time";
    const sortDir = searchParams.get("sortDir") || "desc";
    const selectedDays = parseSelectedDays(searchParams.get("days"));

    if (isLegacySummaryMode()) {
        return getLegacySummaryResponse({
            start,
            end,
            hasExplicitDateFilter,
            page,

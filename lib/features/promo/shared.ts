...
export function buildPromoInsertValues(input: PromoCodeInput) {
    ...
    return {
        ...
        minPurchase: input.minOrderAmount > 0 ? String(input.minOrderAmount) : null,
        maxDiscount: input.maxDiscount > 0 ? String(input.maxDiscount) : null,
...
export function buildPromoUpdateValues(input: PromoUpdateInput) {
    ...
    if (input.minPurchase !== undefined) updateData.minPurchase = parseOptionalDecimal(input.minPurchase);
    if (input.maxDiscount !== undefined) updateData.maxDiscount = parseOptionalDecimal(input.maxDiscount);
